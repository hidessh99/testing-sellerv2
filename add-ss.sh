...
},{"password": "'""$user""'","method": "'""$cipher""'","email": "'""$user""'"' /etc/xray/ss-ws.json
sed -i '/#ssgrpc$/a\### '"$user $exp"'\
},{"password": "'""$user""'","method": "'""$cipher""'","email": "'""$user""'"' /etc/xray/ss-gprc.json
echo $cipher:$user > /tmp/log
shadowsocks_base64=$(cat /tmp/log)
echo -n "${shadowsocks_base64}" | base64 > /tmp/log1
shadowsocks_base64e=$(cat /tmp/log1)
shadowsockslink="ss://${shadowsocks_base64e}@$domain:$tls?plugin=xray-plugin;mux=0;path=/hidessh-ssws;host=$domain;tls#${user}"
...
echo -e "Path : /hidessh-ssws" | tee -a /etc/log-create-user.log
